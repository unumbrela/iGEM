...
if __name__ == '__main__':
    ...
    for e in range(args.clip_epoches):
        ...
        # 训练循环添加进度条
        train_loop = tqdm(train_dataloader, desc=f"Epoch {e+1}/{args.clip_epoches} [Training]")
        for text, pep in train_loop:
            ...
            e_loss += loss.item() # 使用 .item() 获取标量值
            loss.backward()
            optimizer1.step()
            optimizer2.step()
            ...
            # 在进度条上显示实时损失
            train_loop.set_postfix(loss=e_loss / len(train_dataloader))

        print(f"epoch :{e + 1}, clip training loss: {e_loss / len(train_dataloader)}")

        valid_loss = 0
        
        # 验证循环添加进度条
        valid_loop = tqdm(valid_dataloader, desc=f"Epoch {e+1}/{args.clip_epoches} [Validation]")
        for valid_text, valid_pep in valid_loop:
            with torch.no_grad():
                ...
                # 这两行 reshape 似乎不是必须的，因为InfoNCE应该能处理
                ...
                valid_loss += loss.item() # 使用 .item()
                
                # 在进度条上显示实时验证损失
                valid_loop.set_postfix(loss=valid_loss / len(valid_dataloader))
                
        print(f"epoch :{e + 1}, clip valid loss: {valid_loss / len(valid_dataloader)}")
